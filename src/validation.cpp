...
void append_diagnostic(std::vector<Diagnostic>& diagnostics,
                       std::string code,
                       Severity severity,
                       std::string message,
...
ValidationResult validate_trace(const TraceResult& trace, const ValidationPolicy& policy) {
    ...
    for (const auto& node : trace.graph.nodes()) {
        if (node.kind == NodeKind::requirement) {
            if (policy.missing_test.enabled && !connected_to_kind(trace.graph, node.id, NodeKind::test)) {
                append_diagnostic(result.diagnostics,
                                  "validation.missing_test_evidence",
                                  policy.missing_test.severity,
                                  "requirement has no linked test evidence: " + node.id,
                                  node.source);
            }
            if (policy.missing_implementation.enabled &&
                !connected_to_kind(trace.graph, node.id, NodeKind::source)) {
                append_diagnostic(result.diagnostics,
                                  "validation.missing_implementation_evidence",
                                  policy.missing_implementation.severity,
                                  "requirement has no linked implementation evidence: " + node.id,
                                  node.source);
            }
        }

        if (policy.missing_coverage.enabled &&
            (node.kind == NodeKind::source || node.kind == NodeKind::test) &&
            !connected_to_kind(trace.graph, node.id, NodeKind::coverage)) {
            append_diagnostic(result.diagnostics,
                              "validation.missing_coverage_evidence",
                              policy.missing_coverage.severity,
                              "trace node has no linked coverage evidence: " + node.id,
                              node.source);
        }

        if (policy.failed_test.enabled && node.kind == NodeKind::test &&
            node.evidence_state == EvidenceState::failed) {
            append_diagnostic(result.diagnostics,
                              "validation.failed_test_evidence",
                              policy.failed_test.severity,
                              "linked test evidence is failing: " + node.id,
                              node.source);
        }

        if (policy.static_finding.enabled && node.kind == NodeKind::finding &&
            has_any_link(trace.graph, node.id)) {
            append_diagnostic(result.diagnostics,
                              "validation.static_analysis_finding",
                              policy.static_finding.severity,
                              "linked static-analysis finding: " + node.id,
                              node.source);
        }
    }
