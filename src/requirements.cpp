...
[[nodiscard]] bool starts_fence(std::string_view line, char marker) noexcept {
    line = trim(line);
    std::size_t count = 0;
...
[[nodiscard]] std::vector<Heading> scan_headings(std::string_view content) {
...
[[nodiscard]] std::size_t find_token(std::string_view text, std::string_view token) noexcept {
...
[[nodiscard]] std::string normalized_title(std::string_view heading, std::string_view id) {
    ...
    erase_token(id);
    erase_token("@req");
    const auto value = trim(title);
    return std::string{value};
}

[[nodiscard]] std::string trim_body(std::string_view body) {
...
[[nodiscard]] std::string_view requirement_candidate(std::string_view heading) noexcept {
...
RequirementParseResult parse_requirements(std::span<const RequirementDocument> documents) {
    ...
    for (const auto& document : documents) {
        ...
        for (std::size_t index = 0; index < headings.size(); ++index) {
            ...
            const auto body = body_begin <= body_end ? document.content.substr(body_begin, body_end - body_begin)
                                                     : std::string_view{};
            ...
            const auto duplicate = std::find_if(result.requirements.begin(), result.requirements.end(),
                                                [&requirement](const Requirement& existing) {
                                                    return existing.id == requirement.id;
                                                });
    ...
    for (const auto& requirement : requirements) {
        if (!is_requirement_id(requirement.id)) {
            return std::unexpected(Error{ErrorCode::invalid_requirement_id,
                                         "cannot allocate an ID from malformed requirement ID " + requirement.id,
                                         requirement.source});
        }
