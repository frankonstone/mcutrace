...
std::optional<std::uint8_t> parse_evidence(std::string_view heading, bool& valid) {
    ...
    if (end == std::string_view::npos) { valid = false; return std::nullopt; }
    ...
    while (pos <= values.size()) {
        ...
        if (token == "test") mask |= evidence_mask(EvidenceExpectation::test);
        else if (token == "implementation") mask |= evidence_mask(EvidenceExpectation::implementation);
        else if (token == "coverage") mask |= evidence_mask(EvidenceExpectation::coverage);
        else if (token == "build") mask |= evidence_mask(EvidenceExpectation::build);
        else if (token == "none") saw_none = true;
        else { valid = false; return std::nullopt; }
        if (comma == std::string_view::npos) break;
        pos = comma + 1;
    }
    if (saw_none && mask != 0) { valid = false; return std::nullopt; }
    return mask;
}
...
std::string normalized_title(std::string_view heading, std::string_view id) {
    ...
    if (evidence != std::string::npos) {
        ...
        if (end != std::string::npos) title.erase(evidence, end - evidence + 1);
    }
    const auto erase_token = [&title](std::string_view token) {
        const auto pos = title.find(token);
        if (pos != std::string::npos) title.erase(pos, token.size());
    };
    erase_token(id);
    erase_token("@req");
...
std::string_view requirement_candidate(std::string_view heading) noexcept {
    for (std::size_t i = 0; i + 4 <= heading.size(); ++i) {
        if (heading.substr(i, 4) == "REQ-") {
            auto end = i + 4;
            while (end < heading.size() && std::isalnum(static_cast<unsigned char>(heading[end])) != 0) ++end;
...
bool is_requirement_id(std::string_view text) noexcept {
    if (text.size() != 8 || text.substr(0, 4) != "REQ-") return false;
    for (std::size_t index = 4; index < text.size(); ++index) if (text[index] < '0' || text[index] > '9') return false;
    return true;
}
...
RequirementParseResult parse_requirements(std::span<const RequirementDocument> documents) {
    ...
    for (const auto& document : documents) {
        ...
        for (std::size_t index = 0; index < headings.size(); ++index) {
            ...
            if (candidate.empty()) {
                if (marked) add_malformed(result, document, heading, "@req marker requires a REQ-NNNN identifier");
                continue;
            }
            if (!is_requirement_id(candidate)) { add_malformed(result, document, heading, candidate); continue; }
            ...
            for (std::size_t next = index + 1; next < headings.size(); ++next) {
                if (headings[next].level <= heading.level) { body_end = headings[next].line_begin; break; }
            }
...
std::expected<std::string, Error> next_requirement_id(std::span<const Requirement> requirements) {
    ...
    for (const auto& requirement : requirements) {
        if (!is_requirement_id(requirement.id)) return std::unexpected(Error{ErrorCode::invalid_requirement_id,
    ...
    if (maximum >= 9999) return std::unexpected(Error{ErrorCode::requirement_id_space_exhausted,
        "no canonical REQ-NNNN identifier remains after REQ-9999", std::nullopt});
    const auto value = maximum + 1;
    std::array<char, 9> buffer{};
    buffer[0] = 'R'; buffer[1] = 'E'; buffer[2] = 'Q'; buffer[3] = '-';
